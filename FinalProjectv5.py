...
class CNN:
    def __init__(self, image_height, image_width, channels, num_classes):
        ...
        # tochange here
        self.conv_layer_1 = tf.keras.layers.Conv2D(self.input_layer, filters=64, kernel_size=[2, 2], padding="same",
                                             activation=tf.nn.relu)
        self.pooling_layer_1 = MaxPooling2D(self.conv_layer_1, pool_size=[2, 2], strides=2)
        self.flattened_pooling = Flatten(self.pooling_layer_1)
        self.dense_layer = Dense(self.flattened_pooling, 1024, activation=tf.nn.relu)
        self.dropout = Dropout(self.dense_layer, rate=0.4, training=True)
        self.output = Dense(self.dropout, num_classes)
        ...
        self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.0025)
